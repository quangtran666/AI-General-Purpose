...
public record DocumentsDto(int DocumentId, string DocumentName, int? FolderId, string? FolderName);

internal class GetDocumentsQueryHandler(ApplicationDbContext applicationDbContext) : IRequestHandler<GetDocumentsQuery, List<DocumentsDto>>
{
    public async Task<List<DocumentsDto>> Handle(GetDocumentsQuery request, CancellationToken cancellationToken)
    {
        return await applicationDbContext.Documents
            .AsNoTracking()
            .Include(x => x.Folder)
            .Select(x => new DocumentsDto(x.Id, x.Name, x.FolderId, x.Folder.Name))
